...
export async function getLeads(query = '') {
  ...
  let request = supabase.from('leads').select('id,name,category,city,state,phone,email,website,address,open_hours,score,pipeline_status,created_at,updated_at,external_place_id,source').eq('organization_id', organizationId).order('updated_at', { ascending: false }).limit(100)
...
export type LeadRow = {
  id: string; name: string; category: string; city: string; state: string; phone: string | null; email: string | null; website: string | null; address: string | null; source?: string;
...
export async function getLeadDetail(leadId: string) {
  ...
  const [{ data: notes }, { data: activities }, { data: tasks }, { data: invoices }, { data: simulations }, { data: intakes }] = await Promise.all([
    ...
    supabase.from('public_intake_submissions').select('id,upload_path,original_filename,mime_type,upload_state,reviewed_at,reviewed_by,invoice_id,consent_at,consent_version,consent_text').eq('lead_id', leadId).eq('organization_id', organizationId).order('created_at', { ascending: false }).limit(1),
  ])
  const intake = (intakes ?? [])[0] as { id: string; upload_path: string; original_filename: string; mime_type: string; upload_state: string; reviewed_at: string | null; reviewed_by: string | null; invoice_id: string | null; consent_at: string; consent_version: string; consent_text: string } | undefined
  const signedInvoiceUrl = intake?.upload_state === 'RECEBIDO'
    ? (await supabase.storage.from('invoices').createSignedUrl(intake.upload_path, 300)).data?.signedUrl ?? null
    : null
  return {
    ...
    publicIntake: intake ? { ...intake, signedUrl: signedInvoiceUrl } : null,
  }
}
