...
import { PublicIntakeLinkForm } from '@/components/settings/public-intake-link-form'
...
export default async function SettingsPage() {
  ...
  const [{ data: rules }, { data: templates }, { data: links }, { data: canManageLink }] = await Promise.all([
    ...
    supabase.from('public_intake_links').select('code,active').eq('organization_id', organizationId).order('created_at', { ascending: false }).limit(1),
    supabase.rpc('is_organization_owner', { p_organization_id: organizationId }),
  ])
  const currentLink = links?.[0] as { code: string; active: boolean } | undefined
  ...
  return <main className="page-content"><div className="page-heading"><div><div className="eyebrow">Seu workspace</div><h1 className="page-title">Configurações</h1><p className="page-subtitle">Mantenha seu perfil e as regras comerciais atualizados.</p></div></div>
    ...
    <div className="section-gap"><Card><CardHeader><div><h2 className="card-title">Captação pública</h2><p className="card-copy">Compartilhe um formulário para receber empresas e faturas diretamente no seu workspace.</p></div></CardHeader><CardContent><PublicIntakeLinkForm initialLink={currentLink ?? null} canManage={Boolean(canManageLink)} /></CardContent></Card></div>
  </main>
}
