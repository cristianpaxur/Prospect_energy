...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: lead }, { data: invoices }] = await Promise.all([
    supabase.from('leads').select('id,name').eq('id', id).eq('organization_id', organizationId).maybeSingle(),
